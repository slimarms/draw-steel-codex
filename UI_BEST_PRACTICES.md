...
These are available but less commonly used: `gui.SearchInput`, `gui.FancyInput`, `gui.ColorPicker`, `gui.TreeNode`, `gui.ContextMenu`, `gui.ContextMenuItem`, `gui.Border`, `gui.PrettyBorder`, `gui.DialogBorder`, `gui.LoadingIndicator`, `gui.PercentSlider`, `gui.CurrencyEditor`, `gui.AudioEditor`, `gui.Curve`.
...
- **Parent state**: `"parent:hover"`, `"parent:press"`, `"parent:hover-linger"` -- style based on parent's state
...
Toggle the `collapsed` class to show/hide panels. Never destroy and recreate panels for simple visibility changes:
...
- Outlined box: `bgimage = "panels/square.png"` with `borderWidth`, `borderColor`, `cornerRadius`
- Filled rect: `bgimage = true` (filled with `bgcolor`, no specific image texture)
- Transparent: `bgcolor = "clear"`
...
```lua
gui.Panel{
    bgimage = "panels/square.png", -- or simply = true
    bgcolor = "clear",
    borderWidth = 1,
    borderColor = GOLD,
    cornerRadius = 6,
